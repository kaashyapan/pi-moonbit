...
The loop is expected to yield a value; in current syntax, add a `nobreak`
branch.
...
the loop lacks a branch to provide that value. In current MoonBit syntax, that
branch is written as `nobreak`.
...
`nobreak` branch that specifies what value to return when the loop completes
normally. This is because:

1. The loop body itself cannot yield a value (unlike `loop` expressions)
2. Without a `nobreak` branch, there's no way to determine what value should be
