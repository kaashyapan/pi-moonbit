...
number. They are accepted in places such as the `warnings` field in `moon.pkg`
and the `#warnings` attribute. This error happens when the warning list cannot
be parsed, for example because it contains an unknown warning name or malformed
syntax.
