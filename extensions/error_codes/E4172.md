...
This error happens when a `re"..."` literal does not follow MoonBit's regex
syntax. Regex literals are checked at compile time, so syntax errors,
unsupported assertions, invalid ranges, and duplicate capture names are
