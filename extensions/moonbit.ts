...
| 'moon_test'             | Run tests for the current module or a scoped package.                | After 'moon_check' is clean. One call per package. Only packages that changed. |
| 'moon_fmt_info'         | Formats source in place, regenerates public interface (.mbti) files. | Before handing off a change.                                          |
...
| 'moon_analyze'          | Public API usage counts for a package.                               | Instead of grep, to estimate usage before changing a signature.       |
| 'moon_doc'              | Search exported APIs and docs, workspace and dependencies.           | Before using an API you're not certain of the current signature.      |
| 'moon_explain_error'    | Explains a compiler error code.                                      | Before guessing at a fix for an error code you haven't seen explained. |

### Standard workflow
1. Edit source files ('.mbt').
2. 'moon_check' — fix errors before moving on.
3. 'moon_test' — fix failures before moving on.
4. 'moon_fmt_info' before handoff.
...
- If a 'moon_check' error code's message isn't enough to know the fix, call 'moon_explain_error'
  with that code before guessing.
- If '/ moon - doctor' reports the toolchain isn't reachable, say so — do not work around it by
